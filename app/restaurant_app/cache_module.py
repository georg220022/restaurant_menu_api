...
class CacheMenu:
    ...
    def set_menu(
    ...
    ) -> Optional[dict | list | JSONResponse]:
        ...
        if response_data == "NotFound":
            ...
            return JSONResponse(content=cls.menu_404, status_code=404)
        if response_data:
            if menu_id:
...
class CacheSubMenu:
    ...
    ) -> Optional[dict | JSONResponse]:
        """Метод сохранения ответа из БД в кеш для подменю"""
        if response_data == "NotFound":
            # Что бы не кешировать постоянно NotFound на любой новый
            # не существующий id, будем отдавать шаблон
            return JSONResponse(content=cls.sub_menu_404, status_code=404)
        if response_data:
            if sub_menu_id:
                cache_redis.hmset(
...
class CacheDish:
    ...
    def set_dish(
        ...
        if response_data == "NotFound":
            #  Что бы не кешировать постоянно NotFound на любой новый
            #  не существующий id, будем отдавать шаблон
            return JSONResponse(content=cls.dish_404, status_code=404)
        if response_data:
            if dish_id:
                cache_redis.hmset(
                    f"menu_{menu_id}_sub_menus_{sub_menu_id}_dish_{dish_id}",
